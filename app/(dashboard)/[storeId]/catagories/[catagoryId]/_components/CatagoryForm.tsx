...
export const CatagoryForm: React.FC<CatagoryFormProps> = ({
  initialData,
  billboards,
}) => {
  ...
  const description = initialData ? "Edit a catagory." : "Add a new catagory";
  ...
  const onSubmit = async (data: CatagoryFormValues) => {
    try {
      ...
      if (initialData) {
        ...
        await axios.post(`/api/${params.storeId}/catagories`, data);
      }
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error(
        "Make sure you removed all products using this caatagory first."
      );
    } finally {
      setLoading(false);
      setOpen(false);
